...
The `gamlss` name stands for generalized additive models for location (i.e., $\mu$), scale (i.e., $\sigma$), and shape (i.e., $\nu$ and $\tau$). This package gives researchers the ability to use a wide variety of models, including beta regression models. A number of algorithms to estimate the coefficients can be used, described in detail by @stasinopoulos2017flexible. To first demonstrate that the package works according to the parameterization discussed in the Statistical Background section, I will generate a zero-and-one inflated beta distribution and estimate the parameters with an intercepts-only regression using the `gamlss` function. Then, I will demonstrate how to model prejudice as a function of ideology using real data.  
...
```{r echo = TRUE}
# define the inverse of the logit link function
# the exp function is the inverse of the log link
...
## Modeling Prejudice from Political Ideology
